...
bool DatFile::Load(const char* filename, ID3D11Device* device11, ID3D11DeviceContext* device11Context)
{
	...
	for (int i = 0; i < m_datfile->m_MaterialSuu; i++) {
		if (m_datfile->m_DecalTex[i] != "null" && m_datfile->m_DecalTex[i] != "none") {
			...
			bool sts = CreateSRVfromWICFile(m_datfile->m_DecalTex[i].c_str(),		// WICファイルからシェーダーリソースビューを作成する
				device11,
				device11Context,
				&m_tex,
				&m_srv[i]);
	...
	for (int i = 0; i < m_datfile->m_MaterialSuu; i++) {
		...
		for (int j = 0; j < m_datfile->m_SubsetSuu; j++) {
			if (i == m_datfile->m_Subset[j].m_MaterialIdx) {
				...
				subsetidx = j;
				device->DrawIndexed(
					//					m_datfile->m_Subset[subsetidx].m_VertexSuu,	// 描画するインデックス数(Face*3)
					m_datfile->m_Subset[subsetidx].m_FaceCount * 3,	// 描画するインデックス数(Face*3)
