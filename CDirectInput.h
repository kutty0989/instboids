...
private:
	...
	DIMOUSESTATE2			m_MouseStateRelase;	// マウスの状態
...
public:
	...
	bool GetMouseLButtonRelease() const {
		if ((m_MouseState.rgbButtons[0] & 0x80) == 0 && (mouseStateOld.rgbButtons[0] & 0x80)!=0)
		{
			return true;
		}
		else {
	...
	{
		if ((m_keybuffer[keyno] & 0x80) == 0 && (m_oldkeybuffer[keyno] & 0x80) != 0)
		{
			return true;
		}
		return false;
