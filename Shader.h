...
bool CreateInstanceVertexBufferWrite(
	ID3D11Device* device,
	unsigned int stride,				// １頂点当たりバイト数
	unsigned int vertexnum,				// 頂点数
	void* vertexdata,					// 頂点データ格納メモリ先頭アドレス
...
bool CreateHullShader(ID3D11Device* device,							//ハルシェーダーオブジェクトを生成
	const char* szFileName,
	LPCSTR szEntryPoint,
	LPCSTR szShaderModel,
	ID3D11HullShader** ppHullShader);

bool CreateDomainShader(ID3D11Device* device,						// ドメインシェーダーオブジェクトを生成
	const char* szFileName,
	LPCSTR szEntryPoint,
	LPCSTR szShaderModel,
