...
// ハルシェーダーオブジェクトを生成する
//--------------------------------------------------------------------------------------
bool CreateHullShader(ID3D11Device* device,
	const char* szFileName,
	LPCSTR szEntryPoint,
	LPCSTR szShaderModel,
	...
	// ハルシェーダーを生成
	hr = device->CreateHullShader(ShaderObject, ShaderObjectSize, nullptr, ppHullShader);
	if (FAILED(hr))
	{
		if (pBlob)pBlob->Release();
		return false;
...
//--------------------------------------------------------------------------------------
// ドメインシェーダーオブジェクトを生成する
//--------------------------------------------------------------------------------------
bool CreateDomainShader(ID3D11Device* device,
	const char* szFileName,
	LPCSTR szEntryPoint,
	LPCSTR szShaderModel,
	...
	// ドメインシェーダーを生成
	hr = device->CreateDomainShader(ShaderObject, ShaderObjectSize, nullptr, ppDomainShader);
	if (FAILED(hr))
	{
		if (pBlob)pBlob->Release();
		return false;
...
bool CreateGeometryShader(ID3D11Device* device, const char* szFileName, LPCSTR szEntryPoint, LPCSTR szShaderModel, ID3D11GeometryShader** ppGeometryShader)
{
	HRESULT hr;
...
bool CreateInstanceVertexBufferWrite(ID3D11Device* device, unsigned int stride, unsigned int vertexnum, void* vertexdata, ID3D11Buffer** pVertexBuffer)
{
	...
	bd.Usage = D3D11_USAGE_DYNAMIC;							// バッファ使用方法
	bd.ByteWidth = stride * vertexnum;						// バッファの大きさ
	bd.BindFlags = D3D11_BIND_SHADER_RESOURCE;		// 頂点バッファ
	bd.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;				// CPUアクセス可能
