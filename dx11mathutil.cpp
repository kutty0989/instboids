...
/// <returns></returns>
float GetKakudo(float x, float y)
{
	float NextSide = x;//
	float OppositeSide = y;//

	if ((NextSide == 0.0f) && (OppositeSide == 0.0f))
	{
		return 0.0f;
	}

	else if (NextSide == 0.0f)
	{
		if (OppositeSide > 0.0f)
		{
			return 90.0f;
		}
		else if (OppositeSide < 0.0f)
		{
			return 270.0f;
		}
	...
	//else
	//{
	//	float radian = atan2f(OppositeSide, NextSide);
	...
	{
		float radian = atan2f(OppositeSide, NextSide);
		float rad = XMConvertToDegrees(radian);
		rad += 360.0f;
		return rad;
	}
	else
...
float GetcosC(float x, float y)
{
	float NextSide = x;//
	float OppositeSide = y;//

	float syahen = sqrtf(x * x + y * y);
