...
// スタティック　メンバー
//-----------------------------------------------------------------------------
const char* Application::WINDOW_TITLE = "ゾンビランド";
...
unsigned long Application :: MainLoop()
{
	...
	while (window->ExecMessage()) {
		...
		Game::GameInput(delta_time);		// ｹﾞｰﾑ入力	
		Game::GameUpdate(delta_time);		// ｹﾞｰﾑ更新
