...
bool CDirectXGraphics::Init(HWND hWnd, unsigned int Width, unsigned int Height, bool fullscreen)
{
   ...
   // 機能レベル
	...
	//m_lpImmediateContext->RSSetState(m_cissorState.Get());
	...
	/*vp[1].Width = (FLOAT)Width;
	vp[1].Height = (FLOAT)Height / 2;
	vp[1].MinDepth = 0.0f;
	vp[1].MaxDepth = 1.0f;
	vp[1].TopLeftX = 0;
	vp[1].TopLeftY = (FLOAT)Height /2;*/
...
//	hr = DirectX::CreateWICTextureFromFile(m_lpDevice.Get(),_T("Sammp.png"), &m_pTexture, &m_pHeightTexture);
	

	////テクスチャー
	//hr = m_lpDevice->CreateShaderResourceView(m_pTexture,&srvd,&m_pHeightTexture);
	//if (FAILED(hr))
	//	return hr;
	//	
	...
	//m_pDevice->CreateSamplerState(&SamDesc, &m_pSamPoint);
...
void CDirectXGraphics::TurnOffScissor()
{
	m_lpImmediateContext->RSSetState(m_rasterState.Get());
...
void CDirectXGraphics::TurnOnLeftScissor()
{
	m_lpImmediateContext->RSSetState(m_cissorState.Get());
	UINT count = 1;
	D3D11_RECT rect;
	...
	rect.right = static_cast<LONG>(Application::CLIENT_WIDTH/ 2.0f);
	rect.top = static_cast<LONG>(Application::CLIENT_HEIGHT / 2.f);
	rect.bottom = static_cast<LONG>(Application::CLIENT_HEIGHT);

	m_lpImmediateContext->RSSetScissorRects(count, &rect);
}

void CDirectXGraphics::TurnOnRightScissor()
{
	m_lpImmediateContext->RSSetState(m_cissorState.Get());
	UINT count = 1;
	D3D11_RECT rect;

	rect.left = static_cast<LONG>(Application::CLIENT_WIDTH / 2.0f);
	rect.right = static_cast<LONG>(Application::CLIENT_WIDTH);
	rect.top = static_cast<LONG>(Application::CLIENT_HEIGHT / 2.f);
	rect.bottom = static_cast<LONG>(Application::CLIENT_HEIGHT);
